...
Returns <- diff(log(Cl(ASIANTILES)))  # Using log returns for better statistical properties
...
acf_plot <- acf(Returns_ts, main = "Autocorrelation Function (ACF) for ASIANTILES Daily Returns",
...
pacf_plot <- pacf(Returns_ts, main = "Partial Autocorrelation Function (PACF) for ASIANTILES Daily Returns",
     lag.max = 30,
     ci.col = "blue")
dev.off()
