...
# Fetch data for ARVSMART.NS from Yahoo Finance
ARVSMART <- getSymbols("ARVSMART.NS", src = "yahoo", from = "2020-04-01", to = "2024-09-30", 
                  verbose = FALSE, auto.assign = FALSE)

# Calculate daily returns
Returns <- diff(log(Cl(ARVSMART)))  # Using log returns for better statistical properties
...
acf_plot <- acf(Returns_ts, main = "Autocorrelation Function (ACF) for ARVSMART Daily Returns",
...
pacf_plot <- pacf(Returns_ts, main = "Partial Autocorrelation Function (PACF) for ARVSMART Daily Returns",
     lag.max = 30,
     ci.col = "blue")
dev.off()
